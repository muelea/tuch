...
# https://github.com/nkolot/SPIN/blob/master/datasets/preprocess/mpi_inf_3dhp.py
...
def train_data(dataset_path, out_path, joints_idx, scaleFactor,
               extract_img=False, fits_3d=None):
    ...
    if fits_3d is not None:
        ...
        pose=fits_3d['pose']
        shape=fits_3d['shape']
        # for mview fits use provided has_smpl value otherwise has smpl
        has_smpl = fits_3d['has_smpl'] if 'has_smpl' in fits_3d.keys() \
            else np.ones(len(imgnames_))
...
def mpi_inf_3dhp_extract(dataset_path, out_path, mode, extract_img=False, static_fits=None):
    ...
    if static_fits is not None:
        ...
        # we use spin fits
        fits_3d = os.path.join(static_fits, 'mpi_inf_3dhp_spin_fits.npz')
    else:
        fits_3d = None
